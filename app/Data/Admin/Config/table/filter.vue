<script setup>
import { DuxFormItem, DuxModalPage, DuxFormLayout, DuxIconPicker, DuxCodeEditor } from '@duxweb/dvha-pro'
    ...
    <DuxFormLayout label-placement="top">
      ...
      <template  v-if="row.type === 'select'">
        <DuxFormItem label="选项">
          <NDynamicInput v-model:value="model.options" :on-create="(item) => ({
          })">
            <template #default="{ value }">
              <div class="flex gap-2 items-center">
                <NInput v-model:value="model.label" placeholder="选项名" />
                <NInput v-model:value="model.value" placeholder="选项值" />
              </div>
            </template>
          </NDynamicInput>
        </DuxFormItem>
