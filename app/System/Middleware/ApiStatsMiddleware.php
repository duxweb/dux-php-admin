...
class ApiStatsMiddleware
{
    ...
    public function __invoke(Request $request, RequestHandler $handler): Response
    {
        ...
        \Core\App::db()->getConnection()->beginTransaction();
        try {
            // PV
            if ($info) {
                $data = [];
            ...
            if (!\Core\App::redis()->get($key)) {
                \Core\App::redis()->setex($key, 86400 - (time() + 8 * 3600) % 86400, 1);
                LogApi::query()->where('id', $id)->increment('uv');
            }
            \Core\App::db()->getConnection()->commit();
        } catch (\Exception $e) {
            \Core\App::db()->getConnection()->rollBack();
            throw $e;
        }
        return $response;
    }
